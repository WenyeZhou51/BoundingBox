...
func _ready():
	object_label = "Locked Door"
	confidence = 0.95
	is_interactable = false
	super._ready()
	
	# Get reference to the lock
	lock_prefab = $LockPrefab
	
	# Get references to door components
...
func convert_to_normal_door():
	if is_unlocked:
		return
	
	print("Converting locked door to normal door...")
	is_unlocked = true
	object_label = "Door"
...
func interact():
	if not is_unlocked or is_disappeared:
		return  # Only interactable when unlocked and not disappeared
