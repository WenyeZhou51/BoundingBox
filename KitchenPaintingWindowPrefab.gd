...
func interact():
	if has_been_inspected:
		return  # No longer interactable after first inspection
...
func close_panel():
	...
	# Transform into window after viewing
	transform_to_window()

func transform_to_window():
	has_been_inspected = true
	object_label = "Big Window"  # Changes to "Big Window" (won't trigger trash-throwing which requires exact "Window")
	is_interactable = false
	print("Painting transformed into: ", object_label)
...
func create_ui_elements():
	...
	ui_panel.size = Vector2(600, 350)
	ui_panel.position = Vector2(-300, -175)
	ui_panel.visible = false
	...
	content_label.size = Vector2(560, 270)
	content_label.bbcode_enabled = true
	content_label.text = """[color=green][font_size=20][center]PAINTING[/center][/font_size]

[font_size=18]An abstract painting with muted dark colors and countless silhouettes. 

The brushstrokes are very chunky and expressive.[/font_size][/color]"""
	content_label.visible = false
	...
	close_button.position = Vector2(500, 300)
