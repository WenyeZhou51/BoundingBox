...
@export var is_smeared: bool = false
@export var loading_duration: float = 1.0  # Loading bar duration in seconds
...
func _ready():
	if is_smeared:
		object_label = "Smeared Plate"
		confidence = 0.85
		is_interactable = false
		interaction_text = ""
	else:
		object_label = "Room Plate"
		confidence = 0.95
...
func interact():
	...
	if is_smeared:
		print("The plate is smeared and unreadable")
		return
...
func create_ui_elements():
	...
	ui_panel.size = Vector2(400, 200)
	ui_panel.position = Vector2(-200, -100)
	ui_panel.visible = false
	...
	loading_bar.size = Vector2(300, 30)
	loading_bar.position = Vector2(-150, -15)
	...
	content_label.size = Vector2(360, 120)
	content_label.bbcode_enabled = true
	# Set the room content text
	var room_text = "Room " + room_number + ": " + occupant_name
	content_label.text = "[center][font_size=24]" + room_text + "[/font_size][/center]"
	content_label.visible = false
	...
	content_label.add_theme_font_size_override("normal_font_size", 20)
	...
	close_button.position = Vector2(310, 160)
